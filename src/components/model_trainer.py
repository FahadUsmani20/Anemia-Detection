...
class ModelTrainer:
    ...
    def create_model(self):
        ...
        try:
            ...
            model.add(Conv2D(32, (2,2), input_shape=(64,64,3), padding="same", activation="relu"))  ##Input Layer
            model.add(MaxPool2D(2,2)) ## MaxPool 
            
            model.add(Conv2D(64, (2,2), padding="same", activation="relu")) ## Hidden Layer 1
            model.add(MaxPool2D(2,2))
            
            model.add(Conv2D(128, (2,2), padding="same", activation="relu"))## Hiden Layer 2
            model.add(MaxPool2D(2,2))

            model.add(GlobalAveragePooling2D())

            model.add(Dense(100, activation="relu")) ## Hiden Layer 3 
    ...
    def initiate_model_trainer(self, train_img, train_lab, test_img, test_lab):
        try:
            ...
            ##            0       1       2
            #             0.2     0.3     0.5     =  max(.2, .3, .5) = .5 -> belongs to 2 
            # Output will be 2 
            ...
            model.save_weights('model/model_weights.weights.h5', overwrite=True)
